...
export default function useXChat(config) {
  ...
  const requestHandlerRef = React.useRef(undefined);
  const [isRequesting, setIsRequesting] = useState(false);
  const [conversationKey, setConversationKey] = useState(originalConversationKey || generateConversationKey());

  // 消息队列：存储会话切换后的待发送消息
  const messageQueueRef = React.useRef(new Map());
  useEffect(() => {
  ...
  const onRequest = useEvent((requestParams, opts) => {
    if (!provider) {
